---
description: Use when creating, editing, or reviewing any API route handler, service, domain logic, or access policy. Applies to files in app/api/, src/domains/, and src/lib/. Triggers on tasks like: building REST endpoints, adding business logic, handling auth, checking ownership, writing Zod schemas, structuring domain services, or wiring Prisma queries.
alwaysApply: false
---
...
- Existing API routes pre-date this pattern and use raw async functions
- Do not refactor existing routes unless you are actively modifying them
- When modifying an existing route, migrate it to the `routeHandler()` pattern
