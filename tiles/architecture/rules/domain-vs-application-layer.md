---
description: Architecture guide for Domain vs Application layer separation
globs: src/application/**/*.ts,src/domain/**/*.ts
alwaysApply: false
---
...
| ---------------------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `getCenterProfileSlug(center)` in `domain/center/center.ts`      | **domain**      | Pure, single entity, no I/O. Rule 1 fails all 4 signals.                                                                                                           |
...
| `ResolveSlugResponse` type in `domain/routing/routing.ts`        | **domain**      | Pure vocabulary describing a resolved slug; consumed by API route, middleware, and `application/routing/resolveSlug.ts`. Rule 2 (vocabulary) + rule 3 (3+ layers). |
