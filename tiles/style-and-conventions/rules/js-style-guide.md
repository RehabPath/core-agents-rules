...
tiles/style-and-conventions/rules/js-style-guide.md
...
For barrel file specifications, follow the `barrel-files` rule from the `recovery/architecture` tile.
...
For testing specifications, follow the `unit-test` rule from the `recovery/testing` tile.
